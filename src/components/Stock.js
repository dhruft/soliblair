    ...
    <div>
        <div className="deck" onClick={handleClick}>
            ...
            <motion.div
                ...
                animate={{ y: stock[1].length===0 ? "7vw" : (stock[1].length===1 ? "9.25vw" : "11.5vw"), opacity: 1 }}
                exit={{ x: 0, opacity: 0 }}
                transition={{default: { duration: 0.5 }}}
                onAnimationComplete={()=>setAnimate(false)}>       
                    {stock[0].length!==0 ? <Card draggable={false} flower={stock[0][stock[0].length-1].flower} number={stock[0][stock[0].length-1].number} col={20}/> : ""}
            </motion.div>)}

        </div>
        <div className="spacer"/>
        <div className="display" style={{marginTop: animate ? "-7.7031vw" : "0vw"}}>
